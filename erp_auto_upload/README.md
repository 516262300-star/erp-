...
- `主图` 固定只读取 `MATERIAL_ROOT/主图` 目录里的顶层图片，至少 1 张，按文件名自然排序上传；为空会直接终止。文件名里的数字前后空格不影响排序，例如 `主图 1` 会排在 `主图2` 前面。
- 不再上传“主图原图”。`B类主图`、`B 类主图`、`无牛皮癣图` 这类主图附属目录不会作为主图上传；如果它们放在 `主图` 目录下，脚本会记录跳过日志。
...
```text
登录 -> 新建商品 -> 填链接标题 -> 填 SKU/规格（完整型号无候选时才用基础型号兜底）-> 上传主图/内容详情图/尺寸图/无色图 -> 触发视频上传并等待上传进度完成 -> 截图 -> 停在保存前
```
