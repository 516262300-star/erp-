...
```text
登录 -> 新建商品 -> 填链接标题 -> 填 SKU/规格（完整型号无候选时才用基础型号兜底）-> 上传主图/主图原图/内容详情图/尺寸图/无色图 -> 触发视频上传并等待上传进度完成 -> 截图 -> 停在保存前
```
