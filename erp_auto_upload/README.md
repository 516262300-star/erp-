...
```text
MATERIAL_ROOT/
├─ *主图*.jpg / *.png        （也支持直接放在素材根目录，例如 B类主图、主图2）
├─ 主图/ 或 1920主图/        *.jpg *.png
├─ 无logo主图/               *.jpg *.png
...
- `主图` 至少 1 张，按文件名自然排序上传。脚本会先读取素材根目录顶层文件名包含“主图”的图片；如果根目录没有可用主图，再读取 `主图/`、`1920主图/` 或其他以“主图”结尾的目录。文件名里的数字前后空格不影响排序，例如 `主图 1` 会排在 `主图2` 前面。
- `B类主图` 会作为主图上传；`C类主图`、`c类主图`、文件名或目录名含 `无牛皮癣` / `无皮癣` 的图片不会作为主图上传。
- `无logo主图`、`主图无logo`、`无logo图`、`无 logo 图`、`去logo主图`、`去 logo 主图`，以及主图目录下的 `无logo主图`、`主图无logo`、`无logo`、`无 logo`、`无logo图`、`无 logo 图`、`去logo主图`、`去 logo 主图`、`原图` 都支持，里面的图片会按文件名自然排序上传到 ERP 的“主图原图”。
...
- `尺寸图` 和 SKU 上架顺序会先按主型号自然排序，再在同一主型号内按孔距数字从小到大排序，例如 `6711-96 -> 6711-128 -> 6711-192 -> 6787-96 -> 6787-128 -> 6787-192`；未识别到数字孔距的项目排在同主型号数字孔距后面，并保持文件名自然排序。
...
- `视频` 是 ERP 的“主图视频”，只选择文件名包含 `1:1`、`1：1`、`1-1`、`800` 的 `.mp4`；多个匹配时取修改时间最新的；没有匹配只记录 warning。主图视频会放在所有图片上传完成后最后上传。视频控件固定使用 ERP 的真实文件框 `input#video_upload_file`，上传后必须写入隐藏字段 `#videosrc[name=video]` 或播放器 `#play_video` 的 `src`，并且播放器 `#play_video` 必须加载出可预览的视频画面；只写入字段但仍是黑色视频框，会判定主图视频上传失败。脚本会先关闭图片上传弹窗、滚动到视频控件、挂载视频文件并触发 `input/change` 事件；如果控件位于商品主表单内，会跳过表单提交以避免误保存商品。如果仍找不到文件框，会报出“视频上传文件框未找到”并列出页面上实际存在的上传相关控件，方便修正 selector。如果 ERP 弹出“上传进度”，脚本会等到弹窗里出现“上传成功”且“确定”按钮可点后，才自动点击“确定”；如果 ERP 不弹窗、只在视频按钮旁显示“上传中/上传成功”，脚本会继续等待视频字段写入，最长按视频上传超时时间等待，避免服务器处理较慢时误判失败。如果保存后 ERP 没有生成视频，优先检查视频大小、编码和服务器上传大小限制。脚本会对超过 80 MB 的视频输出 warning。
...
- 如果规格段写在颜色前面，例如 `9096-28直径-古铜色`，脚本会直接解析为 ERP 型号 `9096-28直径`，不会再重复拼成 `9096-28直径-28直径`。
- 上架填写型号时，脚本会先等待 ERP 精确候选；像 `2701-96 孔距` 这类素材名会同时按 ERP 简写 `2701-96` 查找，并把 `2701-96` 视为精确候选；如果精确候选仍未出现，再用基础数字型号搜索 ERP 唯一候选。例如文件名解析为 `9091-单孔`，ERP 只有 `9091-27直径` 时会直接使用 `9091-27直径`。
...
```text
登录 -> 新建商品 -> 填链接标题 -> 填 SKU/规格（完整型号无候选时才用基础型号兜底）-> 上传主图/主图原图/内容详情图/尺寸图/无色图 -> 最后上传主图视频并等待上传进度完成及视频预览画面加载 -> 截图 -> 停在保存前
```
