...
- `视频` 是 ERP 的“主图视频”，只选择文件名包含 `1:1`、`1：1`、`1-1`、`800` 的 `.mp4`；多个匹配时取修改时间最新的；没有匹配只记录 warning。主图视频会放在所有图片上传完成后最后上传。视频控件固定使用 ERP 的真实文件框 `input#video_upload_file`，上传后必须写入隐藏字段 `#videosrc[name=video]` 或播放器 `#play_video` 的 `src`，否则脚本会判定主图视频上传失败。脚本会先关闭图片上传弹窗、滚动到视频控件、挂载视频文件并触发 `input/change` 事件；如果控件位于商品主表单内，会跳过表单提交以避免误保存商品。如果仍找不到文件框，会报出“视频上传文件框未找到”并列出页面上实际存在的上传相关控件，方便修正 selector。如果 ERP 弹出“上传进度”，脚本会等到弹窗里出现“上传成功”且“确定”按钮可点后，才自动点击“确定”。如果保存后 ERP 没有生成视频，优先检查视频大小、编码和服务器上传大小限制。脚本会对超过 80 MB 的视频输出 warning。
