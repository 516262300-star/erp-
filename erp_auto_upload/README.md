...
- `无logo主图`、`主图无logo`、`无logo图`、`无 logo 图`、`去logo主图`、`去 logo 主图`，以及 `主图/无logo主图`、`主图/主图无logo`、`主图/无logo`、`主图/无 logo`、`主图/无logo图`、`主图/无 logo 图`、`主图/去logo主图`、`主图/去 logo 主图`、`主图/原图` 都支持，里面的图片会上传到 ERP 的“主图原图”。根目录下的 `原图` 不会作为主图原图上传，避免误传相机原片。
...
- `无色图` 文件夹里每个主型号只需要一张图，会上传到该主型号下所有规格的“无色图”字段；如果某个主型号找不到对应无色图，只记录 warning 并继续上架。
- `视频` 只选择文件名包含 `1:1`、`1：1`、`1-1`、`800` 的 `.mp4`；多个匹配时取修改时间最新的；没有匹配只记录 warning。
...
- 上架填写型号时，如果文件名解析出的完整型号在 ERP 自动补全里没有候选，会再用基础数字型号搜索；当 ERP 只返回一个同基础型号开头的候选时，脚本会自动选用该 ERP 候选。例如文件名解析为 `9091-单孔`，ERP 只有 `9091-27直径` 时会直接使用 `9091-27直径`。
...
- “开始上架（不保存）”会先完整执行 ERP 核对；全部 SKU 的型号、颜色和成本都通过后，才会继续填写和上传。
...
这个命令不填写标题、不新增 SKU、不上传素材，适合正式上架前先核对。
...
```text
登录 -> 新建商品 -> ERP 核对全部 SKU 型号/颜色/成本 -> 填链接标题 -> 填 SKU/规格 -> 上传主图/内容详情图/尺寸图/无色图/视频 -> 截图 -> 停在保存前
```
