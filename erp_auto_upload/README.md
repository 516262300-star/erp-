...
- `视频` 只选择文件名包含 `1:1`、`1：1`、`1-1`、`800` 的 `.mp4`；多个匹配时取修改时间最新的；没有匹配只记录 warning。视频控件是 ERP 原生文件框，脚本会先把视频挂载到文件框并校验文件名；如果 ERP 弹出“上传进度”，脚本会等到弹窗里出现“上传成功”且“确定”按钮可点后，才自动点击“确定”。如果保存后 ERP 没有生成视频，优先检查视频大小、编码和服务器上传大小限制。脚本会对超过 80 MB 的视频输出 warning。
